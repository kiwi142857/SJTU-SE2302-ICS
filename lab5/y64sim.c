...
reg_t reg_table[REG_NONE] = {{"%rax", REG_RAX}, {"%rcx", REG_RCX}, {"%rdx", REG_RDX}, {"%rbx", REG_RBX},
...
{
    long_t val = 0;
    // get the value of argA and argB
    switch (op) {
    case A_ADD:
        val = argA + argB;
        break;
...
stat_t nexti(y64sim_t *sim)
{
    ...
    if (icode != I_HALT && icode != I_NOP && icode != I_RET && icode != I_JMP && icode != I_CALL) {
        byte_t reg = 0;
    ...
    case I_RRMOVQ: /* 2:x regA:regB */
        ...
        else if (cond_doit(sim->cc, LOW(codefun)))
            set_reg_val(sim->r, regB, get_reg_val(sim->r, regA));
        break;
    ...
    case I_RMMOVQ: /* 4:0 regA:regB imm */
        sim->pc = next_pc;
        set_long_val(sim->m, get_reg_val(sim->r, regB) + valC, get_reg_val(sim->r, regA));
        break;
    case I_MRMOVQ: /* 5:0 regB:regA imm */
        if ((valC + get_reg_val(sim->r, regB)) <= 0) {
            sim->pc = 0;
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, get_reg_val(sim->r, regB) + valC);
            return STAT_ADR;
        }
        sim->pc = next_pc;
        get_long_val(sim->m, get_reg_val(sim->r, regB) + valC, &valC);
    ...
    case I_ALU: /* 6:x regA:regB */
        ...
        sim->pc = next_pc;
        // execute the instruction
        valC = compute_alu(ifun, get_reg_val(sim->r, regA), get_reg_val(sim->r, regB));
        sim->cc = compute_cc(ifun, get_reg_val(sim->r, regA), get_reg_val(sim->r, regB), valC);
        // set result to register
        set_reg_val(sim->r, regB, valC);
        break;
    case I_JMP: /* 7:x imm */
        if (cond_doit(sim->cc, LOW(codefun)))
            sim->pc = valC;
        else
            sim->pc = next_pc;
        break;
    case I_CALL: /* 8:x imm */
        ...
        break;
    case I_RET: /* 9:0 */
        valC = get_reg_val(sim->r, REG_RSP);
        get_long_val(sim->m, valC, &valC);
        sim->pc = valC;
        set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP) + 8);
        if (valC <= 0 || valC >= sim->m->len) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, get_reg_val(sim->r, REG_RSP));
            return STAT_ADR;
        }
        break;
    case I_PUSHQ: /* A:0 regA:F */
        ...
        set_long_val(sim->m, valC, get_reg_val(sim->r, regA));
        if (regA == REG_RSP) {
            set_long_val(sim->m, valC, valC+8);
        }
        sim->pc = next_pc;
        break;
    case I_POPQ: /* B:0 regA:F */
        /* return STAT_INS;  unsupported now, replace it with your implementation */
        get_long_val(sim->m, get_reg_val(sim->r, REG_RSP), &valC);
        set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP) + 8);
        set_reg_val(sim->r, regA, valC);
        sim->pc = next_pc;
        break;
