...
## ID: 522031910299
## Description:
##       1. Adding instruction of iaddq as what we did in partB
##       2. change the logic of predicted f_predPC
##             If the instruction ahead of this one by two, which is in execute session and the valA is not a positive number 
##             && the instruction ahead of this one is IOPQ
##             && this ins is a JXX ins
##                I will choose valP as the next instruction
##             thus I have to set IJXX ins's e_valA as valC if I choose not to jmp.
##             thus I have to set jmp's E_srcB as 1 as a flag that I choose not to jmp instead of 0 other changes
##			   so in bubble process, If I detect M_icode is JXX, and M_Cnd tells that it should jmp and M_valE is 1
##                                      I should send a bubble and change pc
##                                   If I detect M_icode is JXX and M_Cnd tells that it should not jmp and M_valE is 0
##                                      I should send a bubble and change pc
##     
#/* $begin pipe-all-hcl */
####################################################################
...
word f_predPC = [
	f_icode == IJXX && E_icode == IRMMOVQ && E_valA > 0 && D_icode == IOPQ : f_valP;
