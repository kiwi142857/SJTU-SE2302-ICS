    ...
    if (!builtin_cmd(argv)) {
        ...
        if ((pid = fork()) == 0) { /* child runs user job */
            if (sigprocmask(SIG_SETMASK, &prev_one, NULL) < 0)
                unix_error("sigprocmask error");
            if (setpgid(0, 0) < 0) /* set child process a group where consist by itself. */
...
void do_bgfg(char **argv)
{
    ...
    /* check whether the job is valid. */
    if (job == NULL) {
        if (jid != 0) {
...
void sigint_handler(int sig)
{
    /* send signal to fg process to terminate. */
...
void sigtstp_handler(int sig)
{
    /* send signal to fg process to stop. */
    pid_t pid = fgpid(jobs);
    if (pid != 0) {
        sigset_t mask_all, prev_one;
        sigfillset(&mask_all);
